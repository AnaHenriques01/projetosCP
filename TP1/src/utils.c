...
float* initPoints(int N){
    float *points[2] = (float*)malloc(N*2*sizeof(float));
    for(int i = 0; i < N; i++){
...
struct cluster** initClusters(int N, int K){
    ...
    for(int i = 0; i < K; i++){
        ...
        clusters[i]->points[2] = initPoints(size);
...
void init(int N, int K, float* points[2], struct cluster** clusters) {
...
*/
void addToPoints(struct cluster* cluster, float point[2]) {
    if(cluster->number_points >= cluster->max_points){
        cluster->max_points = cluster->number_points++;
        cluster->points[2] = realloc(cluster->points, (cluster->max_points)*2*sizeof(float));
    }
...
void addToClosestCluster(float p[2], struct cluster** clusters, int K){
...
void free_structs(int K, float* points[2], struct cluster** clusters){
    free(points[2]);
